 ...
 */
public class MancalaDataStructure implements java.io.Serializable {

    private static final long serialVersionUID = 6587144797674159139L;
     ...
     */
    public void addPitToStore(int playerNum, int pitNum) {

        Countable store = data.get(storePos(playerNum));
        store.addStones(removeStones(pitNum));
    ...
    /**
     * Generates a string representation of the current state of the Mancala game board.
     * The board includes the number of stones in each pit and the stone counts in the stores.
     *
    ...
    public String toString(){
        ...
        returnString.append("|¯¯|");
        for(int i = 12; i > 6; i--){
            returnString.append(String.format("[%02d]", getNumStones(i)));
        }
        returnString.append("|¯¯|\n");
        try{
            returnString.append(String.format("|%02d|                        |%02d|\n",
             getStoreCount(2), getStoreCount(1)));
